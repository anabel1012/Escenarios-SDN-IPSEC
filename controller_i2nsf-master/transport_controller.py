...
files = ["./xml/h2h/0_h2h_transport_esp_enc_auth_hX.xml", "./xml/h2h/1_h2h_add_sad_in_hX.xml", "./xml/h2h/2_h2h_add_sad_out_hX.xml",
         "./xml/h2h/3_h2h_del_sad_in_out_hX.xml"]    # Templates used by the controller to send the IPsec configuration
                                            # to the nodes
...
def register():
    if request.method == 'POST':
        ...
        log_writeln("Peticion recibida ----> IP Control: " + ip_control + " IP Data: " + ip_data + "\n")
        return sign_up(ip_control, ip_data)


def sign_up(ip_control, ip_data):
    log_writeln("Registro recibido ----> IP Control: " + ip_control + " IP Data: " + ip_data + "\n")
    ...
    if ip_control not in hosts.keys():
        try:
            ...
            return 'ERROR'
        result = create_associations(ip_control, ip_data, m)
        if result:
            log_writeln("Registro completado: nodo ip_control " + ip_control + " ip_data " + ip_data + "\n")
            return 'OK'
        else:
            log_writeln("Registro no completado: nodo ip_control " + ip_control + " ip_data " + ip_data + "\n")
            m.close_session()
            return 'ERROR'
    else:
...
def create_associations(ip_control, ip_data, manager):
        ...
        for ip_control_remote in hosts:
            ...
            try:
                config_local_node = create_initial_config(rule_in, rule_out, rule_fwd, ip_data,
                                                          hosts[ip_control_remote].ip_data, spi_in, spi_out, enc_key, vector, int_key)
            ...
            try:
                config_remote_node = create_initial_config(rule_in, rule_out, rule_fwd,
                                                           hosts[ip_control_remote].ip_data, ip_data, spi_out, spi_in, enc_key, vector, int_key)
            ...
            if config1 and config2 and reg:  # and reg
                ...
                add_ipsec_association(Ipsa(ip_control, ip_data, ip_control_remote, hosts[ip_control_remote].ip_data,
                                           spi_in, spi_out))
                # Create a ipsec association for local node (Remote ---> Local)
                add_ipsec_association(Ipsa(ip_control_remote, hosts[ip_control_remote].ip_data, ip_control,
                                           ip_data, spi_out, spi_in))

            else:
                mutex_register.release()
                return False
        add_host(ip_control, ip_data, manager)
        mutex_register.release()
        return True
    else:
        add_host(ip_control, ip_data, manager)
...
def add_host(ip_control, ip_data, manager):
    mutex_hosts.acquire()
    host = Host(ip_data, manager)
...
def create_random_keys(stringLength=16):
...
def create_initial_config(rule_in, rule_out, rule_fwd, local_address, remote_address, spi_in, spi_out, enc_key, vector, int_key):
...
def create_inbound_config(spi_in, local_address, remote_address, enc_key, vector, int_key):
...
def create_outbound_config(spi_out, local_address, remote_address, enc_key, vector, int_key):
...
def delete_config(local_address, remote_address, spi_in, spi_out, old_enc_key, old_vector, old_int_key):
...
def inbound_rekey(ipsa):
    ...
    config_local_node = create_inbound_config(new_spi_in, ipsa.ip_local_data, ipsa.ip_remote_data, enc_key, vector, int_key)
    m = hosts[ipsa.ip_local_control].manager
    rpc = m.edit_config(target='running', config=config_local_node, test_option='test-then-set')
    message_id1 = rpc.id

    config_remote_node = create_inbound_config(new_spi_out, ipsa.ip_remote_data, ipsa.ip_local_data, enc_key, vector, int_key)
...
def outbound_rekey(ipsa):
    ...
    config_local_node = create_outbound_config(info_rekey.spi_out_new, ipsa.ip_local_data, ipsa.ip_remote_data, enc_key, vector, int_key)
    m = hosts[ipsa.ip_local_control].manager
    rpc = m.edit_config(target='running', config=config_local_node, test_option='test-then-set')
    message_id1 = rpc.id

    config_remote_node = create_outbound_config(info_rekey.spi_in_new, ipsa.ip_remote_data, ipsa.ip_local_data, enc_key, vector, int_key)
    m = hosts[ipsa.ip_remote_control].manager
    rpc = m.edit_config(target='running', config=config_remote_node, test_option='test-then-set')
    message_id2 = rpc.id
...
def delete_rekey(ipsa):
    ...
    config_local_node = delete_config(ipsa.ip_local_data, ipsa.ip_remote_data, ipsa.spi_in, ipsa.spi_out, old_enc_key, old_vector, old_int_key)
    m = hosts[ipsa.ip_local_control].manager
    rpc = m.edit_config(target='running', config=config_local_node, test_option='test-then-set')

    message_id1 = rpc.id

    config_remote_node = delete_config(ipsa.ip_remote_data, ipsa.ip_local_data, ipsa.spi_out, ipsa.spi_in, old_enc_key, old_vector, old_int_key)
    m = hosts[ipsa.ip_remote_control].manager
    rpc = m.edit_config(target='running', config=config_remote_node, test_option='test-then-set')
...
def update_structures(ipsa):
    ...
    ipsa_new_1 = Ipsa(ipsa.ip_local_control, ipsa.ip_local_data, ipsa.ip_remote_control, ipsa.ip_remote_data,
                      info_rekey.spi_in_new, info_rekey.spi_out_new)

    ipsa_new_2 = Ipsa(ipsa.ip_remote_control, ipsa.ip_remote_data, ipsa.ip_local_control, ipsa.ip_local_data,
...
class Host:
    def __init__(self, ip_data, manager):
        self.ip_data = ip_data
        self.manager = manager


class Ipsa:
    def __init__(self, ip_local_control, ip_local_data, ip_remote_control, ip_remote_data, spi_in, spi_out):
